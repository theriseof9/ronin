...
from model_lstm_ode import (
    ODELSTMSeqNetwork,
    BilinearODELSTMSeqNetwork,
)
...
def get_model(args, **kwargs):
    ...
    #     print("TCN Network. Receptive field: {} ".format(network.get_receptive_field()))
        ...
        print("Bilinear ODELSTM Network")
        network = BilinearODELSTMSeqNetwork(
            input_size=_input_channel,
            out_size=_output_channel,
            lstm_size=args.layer_size,
            lstm_layers=args.layers,
            ode_hidden_size=kwargs.get('ode_hidden_size', 64),
            solver=kwargs.get('solver', 'dopri5'),
            **config
        ).to(device)

    else:
        print("Simple ODELSTM Network")
    ...
    try:
        for epoch in range(start_epoch, args.epochs):
            ...
            if not quiet_mode:
                print('-' * 25)
                print(
                    'Epoch {}, time usage: {:.3f}s, loss: {}, vel_loss {}/{:.6f}'.format(
                        epoch,
                        end_t - start_t,
                        train_errs[epoch],
                        train_vel.get_channel_avg(),
                        train_vel.get_total_avg(),
                    )
                )
            ...
            if val_loader:
                ...
                if not quiet_mode:
                    print(
                        'Validation loss: {} vel_loss: {}/{:.6f}'.format(
                ...
                if val_loss < best_val_loss:
                    ...
                    saved_model = True
                    if args.out_dir:
                        model_path = osp.join(args.out_dir, 'checkpoints', 'best_checkpoint.pt')
                        torch.save(
                            {
            ...
            if args.out_dir and not saved_model and (epoch + 1) % args.save_interval == 0:
                # save even without validation
                model_path = osp.join(args.out_dir, 'checkpoints', 'checkpoint_epoch_%d.pt' % epoch)
                torch.save(
                    {
                        'model_state_dict': network.state_dict(),
                        'epoch': epoch,
            ...
            if np.isnan(train_loss):
                print("Invalid value encountered in loss. Stopping training.")
                break
...
def test(args, **kwargs):
    ...
    if args.test_path is not None:
        ...
        with open(args.test_list) as f:
            test_data_list = [
                s.strip().split(',')[0] for s in f.readlines() if len(s) > 0 and s[0] != '#'
            ]
    else:
    ...
    if device.type == 'cpu':
        ...
        checkpoint = torch.load(
            args.model_path, map_location={model_data['device']: args.device}
        )
    ...
    if args.test_list and args.out_dir:
        log_file = osp.join(
            args.out_dir, osp.split(args.test_list)[-1].split('.')[0] + '_log.txt'
        )
    ...
    for idx, data in enumerate(test_data_list):
        ...
        preds = preds.cpu().numpy()[0]  # Remove batch dimension
        ...
        pos_gt, gv_gt, _ = recon_traj_with_preds_global(
            seq_dataset, vel, ind=ind, type='gt', seq_id=idx
        )

        if args.out_dir is not None and osp.isdir(args.out_dir):
            np.save(
                osp.join(args.out_dir, '{}_{}.npy'.format(data, args.type)),
        ...
        if pos_pred.shape[0] < pred_per_min:
            ...
            rte = (
                compute_relative_trajectory_error(
                    pos_pred, pos_gt, delta=pos_pred.shape[0] - 1
                )
                * ratio
            )
        else:
        ...
        rte_all.append(rte)
        print(
            'Sequence {}, Velocity loss {} / {}, ATE: {}, RTE:{}'.format(
